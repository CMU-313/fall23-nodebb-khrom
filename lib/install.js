...
async function createAssignmentPost() {
  const db = require('./database');
  const Topics = require('./topics');
  const [content, numTopics] = await Promise.all([fs.promises.readFile(path.join(__dirname, '../', 'install/data/welcome_assignments.md'), 'utf8'), db.getObjectField('assignment', 'topicCount')]);
  if (!parseInt(numTopics, 10)) {
    console.log('Creating welcome post!');
    await Topics.post({
      uid: 1,
...
async function createAnnouncementPost() {
  const db = require('./database');
  const Topics = require('./topics');
  const [content, numTopics] = await Promise.all([fs.promises.readFile(path.join(__dirname, '../', 'install/data/welcome_announcements.md'), 'utf8'), db.getObjectField('announcement', 'topicCount')]);
  if (!parseInt(numTopics, 10)) {
    console.log('Creating welcome post!');
    await Topics.post({
      uid: 1,
...
async function createCommentsPost() {
  const db = require('./database');
  const Topics = require('./topics');
  const [content, numTopics] = await Promise.all([fs.promises.readFile(path.join(__dirname, '../', 'install/data/welcome_comments.md'), 'utf8'), db.getObjectField('comments', 'topicCount')]);
  if (!parseInt(numTopics, 10)) {
    console.log('Creating welcome post!');
    await Topics.post({
      uid: 1,
